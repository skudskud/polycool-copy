...
class QuickSwapClient:
    ...
    def __init__(self, rpc_url: str = "https://polygon-rpc.com"):
        ...
        logger.info(f"✅ QuickSwap client initialized")
        logger.info(f"   Router: {QUICKSWAP_ROUTER_V2}")
        logger.info(f"   Network: {'Connected' if self.w3.is_connected() else 'Disconnected'}")
    ...
    def get_pol_balance(self, address: str) -> float:
        ...
        try:
            ...
            logger.info(f"💰 POL balance for {address[:10]}...: {balance_pol} POL")
            return float(balance_pol)

        except Exception as e:
            logger.error(f"❌ Error getting POL balance: {e}")
            return 0.0
    ...
    def get_usdc_balance(self, address: str) -> float:
        ...
        try:
            ...
            logger.info(f"💵 USDC.e balance for {address[:10]}...: {balance_usdc} USDC")
            return balance_usdc
    ...
    def get_swap_quote(self, pol_amount: float) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"   Min output (with {MAX_SLIPPAGE_PERCENT}% slippage): {min_usdc_out:.2f} USDC.e")
    ...
    def swap_pol_to_usdc(
        ...
        """
        try:
            logger.info(f"🔄 Executing swap: {pol_amount} POL → USDC.e")
            ...
            logger.info(f"   Gas price: {tx['gasPrice']} wei")
            logger.info(f"   POL amount: {pol_amount}")
            logger.info(f"   Min USDC out: {min_usdc_out}")
            ...
            if receipt['status'] == 1:
                ...
                logger.info(f"   Effective gas price: {receipt['effectiveGasPrice']}")
                return tx_hash.hex()
            else:
                logger.error(f"❌ Swap transaction failed!")
                logger.error(f"   Receipt status: {receipt['status']}")
                logger.error(f"   Gas used: {receipt.get('gasUsed', 'unknown')}")
                # Try to get revert reason if possible
                try:
                    # This might not work for all cases, but let's try
                    revert_reason = self.w3.eth.call(tx, receipt['blockNumber'])
                    logger.error(f"   Revert reason: {revert_reason}")
                except Exception as revert_e:
                    logger.error(f"   Could not get revert reason: {revert_e}")
                return None
    ...
    def auto_swap_excess_pol(
        ...
        try:
            ...
            if swappable_pol <= 0:
                logger.info(f"⚠️ No POL to swap (balance: {pol_balance}, reserve: {reserve_pol})")
                return None

            logger.info(f"🔄 Auto-swapping {swappable_pol} POL (keeping {reserve_pol} POL for gas)")
            ...
            logger.error(f"❌ Error in auto-swap: {e}")
            return None

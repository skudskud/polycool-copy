            ...
            while time.time() - start_time < timeout:
                ...
                if response.value and len(response.value) > 0:
                    ...
                    if status is not None:
                        ...
                        # Accept Confirmed, Finalized, or Processed as success
                        if status.confirmation_status in [Confirmed, Finalized, Processed]:
                            logger.info(f"✅ Transaction confirmed! (status: {status.confirmation_status})")
                            logger.info(f"   View on Solscan: https://solscan.io/tx/{signature}")

                            # Check if transaction has error even if confirmed
                            if status.err:
                                logger.warning(f"⚠️ Transaction has on-chain error: {status.err}")
                                # IMPORTANT: Don't return False for Custom(1) - Jupiter swaps often have this
                                # but still succeed. We'll verify with balance checks instead.
                                logger.warning(f"   Ignoring error - will verify success via balance check")

...
# from config import PRIVATE_KEY, API_CREDENTIALS, AGGRESSIVE_BUY_PREMIUM, AGGRESSIVE_SELL_DISCOUNT
from config import AGGRESSIVE_BUY_PREMIUM, AGGRESSIVE_SELL_DISCOUNT  # Keep trading config only
...
class SpeedTrader:
    ...
    def speed_buy(self, market: Dict, outcome: str = 'yes', amount_usd: float = 2.0) -> Optional[str]:
        ...
        try:
            ...
            if outcome.lower() == 'yes' and len(token_ids) > 0:
                token_id = token_ids[0]
                base_price = float(prices[0]) if len(prices) > 0 else 0.5
                outcome_name = outcomes[0] if len(outcomes) > 0 else 'YES'
            elif outcome.lower() == 'no' and len(token_ids) > 1:
                token_id = token_ids[1] 
                base_price = float(prices[1]) if len(prices) > 1 else 0.5
                outcome_name = outcomes[1] if len(outcomes) > 1 else 'NO'
            else:
                print("❌ Invalid outcome or insufficient token data")
                return None
            ...
            actual_cost = size * aggressive_price
    ...
    def speed_sell(self, market: Dict, outcome: str = 'yes', tokens_to_sell: int = 5) -> Optional[str]:
        ...
        try:
            ...
            # ULTRA-aggressive discount for INSTANT sell
            ultra_aggressive_discount = 0.03  # -3¢ for guaranteed instant sale
            aggressive_price = market_price - ultra_aggressive_discount
            
            # Ensure price doesn't go negative
            aggressive_price = max(0.001, aggressive_price)
            will_receive = tokens_to_sell * aggressive_price
            
            print(f"💰 Market price: ${market_price:.4f}")
            print(f"⚡ ULTRA price: ${aggressive_price:.4f} (-{ultra_aggressive_discount:.3f} = -3¢)")

...
class DeBridgeClient:
    ...
    def get_quote(
        ...
        try:
            ...
            if 'estimation' in quote_data:
                ...
                logger.info(f"   Estimation keys: {list(estimation.keys())}")
                if 'dstChainTokenOut' in estimation:
                    ...
                    logger.info(f"   dstChainTokenOut keys: {list(dst_token.keys())}")
                # Check if there's also a direct 'dst_amount' field
                if 'dst_amount' in estimation:
                    logger.info(f"   Found dst_amount: {estimation['dst_amount']}")
                else:
                    logger.info(f"   No dst_amount field found")
    ...
    def create_order(
        ...
        try:
            ...
            if 'estimation' in order_data:
                logger.info(f"   estimation keys: {list(order_data['estimation'].keys())}")
                estimation = order_data['estimation']
                # Check for different amount fields in create_order response
                if 'dst_amount' in estimation:
                    logger.info(f"   create_order dst_amount: {estimation['dst_amount']}")
                if 'dst_amount_expected' in estimation:
                    logger.info(f"   create_order dst_amount_expected: {estimation['dst_amount_expected']}")
                if 'dstChainTokenOut' in estimation:
                    dst_token_out = estimation['dstChainTokenOut']
                    logger.info(f"   dstChainTokenOut keys: {list(dst_token_out.keys())}")
                    if 'amount' in dst_token_out:
                        logger.info(f"   dstChainTokenOut.amount: {dst_token_out['amount']}")

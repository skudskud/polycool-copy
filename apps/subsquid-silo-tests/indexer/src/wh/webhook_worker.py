...
class CopyTradeEvent(BaseModel):
    """Copy trade webhook event model (from indexer)"""
    tx_id: str
    user_address: str
    ...
    address_type: Optional[str] = None  # onchain, bot_user, external_leader
...
async def receive_copy_trade(event: CopyTradeEvent) -> WebhookResponse:
    """
    Receive copy trade event from indexer (via Redis Bridge).

    This endpoint receives trade events for copy trading.
    Expected payload matches CopyTradeEvent model.

    Note: In production, this may forward to bot's webhook receiver.
    """
    try:
        ...
        if not event.user_address:
            raise ValueError("user_address is required")
        if not event.tx_type:
            raise ValueError("tx_type is required")

        # Log the event
        logger.info(
            f"📥 Copy trade event received: {event.tx_type} for {event.user_address[:10]}... "
            f"(market: {event.market_id}, tx: {event.tx_id})"
        )

        # TODO: Optionally forward to bot's webhook receiver if configured
        # TODO: Optionally store in database for analytics

        metrics.success_count += 1
        return WebhookResponse(
            status="ok",
            message=f"Copy trade event received for {event.user_address[:10]}..."
        )
        ...
        metrics.error_count += 1
        logger.error(f"❌ Copy trade webhook processing error: {e}")
